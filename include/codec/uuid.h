...
class uuid {
 public:
  ...
  inline gsl::span<std::byte const, 16> as_bytes() const {
    return gsl::span<std::byte const, 16>(
  ...
  static bool is_valid_uuid(CharT const* str) noexcept {
    ...
    if (str == nullptr || size == 0) return false;

    if (str[0] == static_cast<CharT>('{')) hasBraces = 1;
    if (hasBraces && str[size - 1] != static_cast<CharT>('}')) return false;
  ...
  static std::optional<uuid> from_string(CharT const* str) noexcept {
    ...
    if (str[0] == static_cast<CharT>('{')) hasBraces = 1;
    if (hasBraces && str[size - 1] != static_cast<CharT>('}')) return {};
  ...
  template <class CharT = char, class Traits = std::char_traits<CharT>,
            class Allocator = std::allocator<CharT>>
  static std::optional<uuid> from_string(
      std::basic_string<CharT, Traits, Allocator> const& str) noexcept {
...
std::basic_ostream<Elem, Traits>& operator<<(
    std::basic_ostream<Elem, Traits>& s, uuid const& id) {
  return s << std::hex << std::setfill(static_cast<Elem>('0')) << std::setw(2)
           << (int)id.data[0] << std::setw(2) << (int)id.data[1] << std::setw(2)
           << (int)id.data[2] << std::setw(2) << (int)id.data[3] << '-'
           << std::setw(2) << (int)id.data[4] << std::setw(2) << (int)id.data[5]
           << '-' << std::setw(2) << (int)id.data[6] << std::setw(2)
           << (int)id.data[7] << '-' << std::setw(2) << (int)id.data[8]
           << std::setw(2) << (int)id.data[9] << '-' << std::setw(2)
           << (int)id.data[10] << std::setw(2) << (int)id.data[11]
           << std::setw(2) << (int)id.data[12] << std::setw(2)
           << (int)id.data[13] << std::setw(2) << (int)id.data[14]
...
class uuid_name_generator {
 ...
 private:
  ...
  void process_characters(char_type const* const characters,
                          size_t const count) {
 ...
 public:
  ...
  uuid operator()() {
    if (get_mac_address()) {
      ...
      memcpy(&data[8], reinterpret_cast<uuids::uuid::value_type*>(&clock_seq),
             2);

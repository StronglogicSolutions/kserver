...
ProcessResult qx(std::vector<std::string> args,
  ...
  fd_set err_file_descriptors{};                  // Mask for error file descriptors
  ...
  // FD_ZERO(&err_file_descriptors);                 // clear
  FD_SET(stdout_fds[0], &read_file_descriptors);   // add stdout to mask
  // FD_SET(stderr_fds[0], &err_file_descriptors);   // add stderr to mask
  ...
  for (;;) {
    ...
    if (num_of_fd_outputs > 0) {                   // output can be read
      if (FD_ISSET(stdout_fds[0], &read_file_descriptors)) {
        ...
        if (result.output.size() < 2) {               // stdout had empty output
          result.output = "Unknown error. Nothing returned from process.";
          result.error = true;
        }
      }
      // if (FD_ISSET(stderr_fds[0], &err_file_descriptors)) { // stderr has output
    ...
    if (result.error) {
      int num_of_fd_outputs = select(
        ...
        NULL,                       // read fd mask
        NULL,                                        // write fd mask
        &err_file_descriptors,                                        // error fd mask
        &time_value
      );

      if (num_of_fd_outputs > 0) {
        if (FD_ISSET(stderr_fds[0], &err_file_descriptors)) { // stderr has output
          printf("ProcessExecutor's forked process returned an error");
          result.output = readFd(stderr_fds[0]);
          result.error = true;
  ...
  std::cout << "Result was " << result.output.size() << " bytes long" << std::endl;

...
typedef std::function<void(std::string, int, std::string, int)> TrackedEventCallback;
/** Manager Interface */
class ProcessManager
{
public:
  virtual void request(std::string_view path, int mask, int client_id, std::vector<std::string> argv) = 0;
  virtual void request(std::string_view path, int mask, int client_id, std::string request_id, std::vector<std::string> argv) = 0;
  ...
  virtual void notifyProcessEvent(std::string status, int mask, int client_id) = 0;
  virtual void notifyTrackedProcessEvent(std::string status, int mask, std::string request_id, int client_id) = 0;
};
...
public:
  ...
  virtual void setEventCallback(TrackedEventCallback f) override { m_tracked_callback = f; }
  ...
  virtual void request(std::string_view path, int mask, int client_socket_fd, std::string request_id, std::vector<std::string> argv) override
  {
    if (path[0] != '\0')
    {
      ProcessDaemon *pd_ptr = new ProcessDaemon(path, argv);
      ...
      if (!process_std_out.empty())
      {
        notifyTrackedProcessEvent(process_std_out, mask, request_id, client_socket_fd);
      }
      delete pd_ptr;
    }
  }
